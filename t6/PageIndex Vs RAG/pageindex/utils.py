...
def extract_json(raw)-> dict | list:
    if raw is None :
        raise ValueError("Respons content is none ")
    ...
    balanced = _extract_balanced_json(candidates[-1])
    if balanced : 
        candidates.append(balanced)

    for candidate in candidates:
        cleaned = _cleanup_json_text(candidate)
        attempts.append(cleaned[:300])

        try:
            return json.loads(cleaned)
        except json.JSONDecodeError as e:
            print("\n======== JSON candidate failed======")
            print("candidate: ")
    ...
    if balanced :
        cleaned = _cleanup_json_text(balanced)
        try : 
            return json.loads(cleaned)
        except json.JSONDecodeError as e:
            raise ValueError (
                f"failed to parse extracted balanced json : {e}\n"
                f"Preview:\n{cleaned[:500]}"
            )from e
    ...
    raise ValueError(
        ...
        f"attempts preview: {attempts}"
    )

def parse_llm_json_response(content, stage_name="unknown_stage"):
    print(f"\n--------{stage_name }----------")
    print("content type: ",type(content))
    print("raw content preview:",repr(content[:500] if isinstance(content, str) else content))

    try: 
        parsed = extract_json(content)
        print(f"[{stage_name}] JSON parse success")
        return parsed
    except Exception as e :
        preview= content[:500] if isinstance(content, str) else str(content)
        print(f"[{stage_name}] JSON parse failed: {e}")
        print(f"[{stage_name}] raw preview: {repr(preview)}")
        raise 
