...
async def check_title_appearance(item, page_list, start_index=1, model=None):
    ...
    text_clean = "".join(page_text.lower().split())

    answer = "yes" if title_clean in text_clean else "no"
...
async def check_title_appearance_in_start(title, page_text, model=None, logger=None):    
    ...
    response = await llm_acompletion(model=model, prompt=prompt)
    response = parse_llm_json_response(response,"check_title_appearance_in_start")
    if logger:
...
def toc_detector_single_page(content, model=None):
    ...
    json_content = parse_llm_json_response(response,"toc_detector_single_page")

    if 'toc_detected' not in json_content:
        print("TOC DETECTOR FAILED")
        print(response)
        return "no"
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    prompt = prompt + '\n Document:\n' + content + '\n Table of contents:\n' + toc
    response = llm_completion(model=model, prompt=prompt)
    json_content = parse_llm_json_response(response,"check_if_toc_extraction_is_complete")
    if 'completed' not in json_content:
...
def check_if_toc_transformation_is_complete(content, toc, model=None):
    ...
    # }}
    # Directly return the final JSON structure. Do not output anything else."""
    print("INSIDE check_if_toc_transformation_is_complete")
    prompt = f"""
    You are given:

    1. A raw table of contents.
    2. A cleaned table of contents.

    Your task is to determine whether the cleaned table of contents contains all major sections from the raw table of contents.
    ...
    {{
        "thinking": "The cleaned TOC contains all major sections.",
        "completed": "yes"
    }}
    ...
    {{
        "thinking": "Several sections are missing.",
        "completed": "no"
    }}
    ...
    # return json_content['completed']
    json_content = parse_llm_json_response(response,"check_if_toc_transformation_is_complete")
    if 'completed' not in json_content:
        print("JSON PARSE FAILED")
        print(response[:1000])
        return "no"
...
def detect_page_index(toc_content, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = parse_llm_json_response(response,"detect_page_index")
...
#     json_content = parse_llm_json_response(response,"toc_index_extractor")    
#     print("\n extracted json : ")
...
def toc_index_extractor(toc , content, model=None):
    print("star toc_index_extractor")
    toc_extractor_prompt="""
    You are a JSON generator.

    You are given :
    1. A table of contents in JSON format.
    2.Several document pages.

    Your task:
    Add " physical_index" only for sections that clearly appear in the provided pages.

    Return ONLY a valid JSON array.
    Do not explain.
    Do nit summarise the document.
    Do not use markdown.
    Do not use ```json blocks.
    The response must start with [ and end with ].
    ...
    If no section can be matched, return [].  
    """
    prompt= toc_extractor_prompt + '\n table of contents:\n'+ str(toc) + '\n Document pages\n'+ content
    print("="*80)
    print("prompt length : ",len(prompt))
    print("content length: ",len(content))
    print("toc length : ",len(str(toc)))
    print("="*80)
    response = llm_completion(model=model,prompt=prompt)
    print("======== type of response ==========")
    print(type(response))
    print(repr(response[:500] if isinstance(response, str) else response))
    try:
        json_content=parse_llm_json_response(response, "toc_index_extractor")
    except Exception:
        retry_promt= prompt + "\n\n Reminder: return only JSON array. No explanation. No prose."
        retry_response = llm_completion(model=model, prompt=retry_promt)
        json_content = parse_llm_json_response(retry_response , "toc_index_extractor_retry")
    if not isinstance(json_content,list):
        raise ValueError("toc_index_extractor must only return a JSON list")
    
    print("\nextracted content")
    print(type(json_content))
    print(json_content[:3] if isinstance(json_content, list) else json_content )
...
def toc_transformer(toc_content, model=None):
    ...
    if if_complete == "yes" and finish_reason == "finished":
        last_complete = parse_llm_json_response(last_complete,"toc_transformer_initial")
    ...
    last_complete = parse_llm_json_response(last_complete,"toc_transformer_final")
...
def page_list_to_group_text(page_contents, token_lengths, max_tokens=1500, overlap_page=0):    
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    print("================================================\n")
    print("="*80)
    print("TYPE: ",type(toc_content))
    print("value : ", toc_content )
    print("="*80)
    if isinstance(toc_content,dict):
        toc_content=[toc_content]
    prompt = prompt + '\nGiven text\n:' + part + '\nPrevious tree structure\n:' + json.dumps(toc_content[-1:], indent=2)
    print("PROMPT TOKENS:", count_tokens(prompt, model))
    response, finish_reason = llm_completion(model=model, prompt=prompt, return_finish_reason=True)
    print("FINISH REASON =", finish_reason)
    print("\n================ CONTINUE RESPONSE ================")
    print(response)
    print("LEN =", len(response))
    print("REPR =", repr(response[:500]))
    print("===================================================\n")
    if response.strip() == "":
    ...
    if finish_reason == 'finished':
        result = parse_llm_json_response(response,"generate_toc_continue")

        if isinstance(result,dict):
            result=[result]
        return result 
    if finish_reason == "length":
        print("Model hit context limit")
        return []
...
def generate_toc_init(part, model=None):
    ...
    print("================================================\n")
    if finish_reason == 'finished':
        result = parse_llm_json_response(response,"generate_toc_init")
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    print("TOTAL TOKENS:", sum(token_lengths))
    print("MAX TOKENS PER GROUP:", 2500)
    ...
    for group_text in group_texts[1:]:
        ...
        print("\nTYPE A:", type(toc_with_page_number))
        print("TYPE B:", type(toc_with_page_number_additional))

        print("\nVALUE A:")
        print(toc_with_page_number)

        print("\nVALUE B:")
        print(toc_with_page_number_additional) 
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    ...
    for page_index in range(start_page_index, min(start_page_index + 5 , len(page_list))):
        main_content += f"<physical_index_{page_index+1}>\n{page_list[page_index][0]}\n<physical_index_{page_index+1}>\n\n"

    print("="*80)
    print("main_content charecters :", len(main_content))

    toc_with_physical_index = toc_index_extractor(toc_no_page_number, main_content, model)
    print("\n after toc_index extractor")
    print(type(toc_with_physical_index))
    print(toc_with_physical_index)
    print()
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    # accuracy, incorrect_results = await verify_toc(page_list,toc_with_page_number,start_index=start_index,N=3,model=opt.model) 
    accuracy = 1.0
    incorrect_results = [] 
    logger.info({
    ...
    if accuracy == 1.0 and len(incorrect_results) == 0:
        print("\n========= meta processor======")
        print("toc entries:",len(toc_with_page_number))

        for i , item in enumerate(toc_with_page_number[:10]):
            print(f"{i}: {item}")
        print("===================\n")
        return toc_with_page_number
    ...
    #     return toc_with_page_number
    if accuracy > 0.6 and len(incorrect_results) > 0:
        print("Skipping fix_incorrect_toc")
        print("\nFINAL TOC:")
        for item in toc_with_page_number:
            print(item)
        return toc_with_page_number
